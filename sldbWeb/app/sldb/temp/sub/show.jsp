<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@page import="com.saturn.sldb.Temporary"%>
	...
	<script type="text/javascript">
		$(function() {
			$('#querySabPersonTable').datagrid({
				...
				url : '<%=request.getContextPath()%>/app/sldb/temp/sub/listSabTemp.action?id=' + <%=request.getParameter("id") %>,
				queryParams : {
					state : '创建'
				},
				idField : 'id',//唯一标识列
				...
				toolbar : [ {
					...
					iconCls : 'icon-add',
					handler : function() {
						window.location.href='<%=request.getContextPath()%>/app/sldb/temp/sub/add.jsp?pid=' + <%=request.getParameter("id")%>;
	...
	<div id="panel" class="easyui-panel" title="户主信息"
		...
		<form id="queryForm" method="post">
			<table>
				<tr align="right">
					<td>户主姓名:</td>
					<td><input id="name" name="name" type="text" readonly="readonly" value="${temporary.name}"></input></td>
					<td></td>
					<td>身份证:</td>
					<td><input id="identify" name="identify" type="text" readonly="readonly" value="${temporary.identify}"></input></td>
					<td></td>
					<td>性别:</td>
					<td><input id="sex" name="sex" type="text" readonly="readonly" value="${temporary.sex}"></input></td>
					<input type="hidden" id="pid" name="pid" value="${temporary.id}"></input>
				</tr>
			</table>
		</form>
	</div>
